...
def test_memory_decorated_functions_has_correct_dynamic_functions(event_loop):
    manager = build_manager(dsn="memory://", loop=event_loop)

    globals()["has_delay_method"] = False

    @manager.task()
    async def task_test_memory_decorated_functions_has_correct_dynamic_functions():
        pass
